...
/// [`RemoteSubscriberRegistry`] / [`RemoteQueryableRegistry`]:
/// liveliness signals are a distinct application surface from
/// pub/sub topology — a consumer that wires "process X is alive"
/// logic does not (and should not) also fire on "process X just
/// subscribed to Y". Keeping the registries split matches zenoh-
/// pico's structural separation and lets consumers reason about
...
mod tests {
    ...
    // ── R121k-4 LivelinessRegistry tests ──────────────────────

    fn decl_token(id: u64, mapping_id: u64, suffix: Option<&str>) -> DeclToken {
        let suffix_owned = suffix.map(str::to_string);
        let suffix_len = suffix.map(|s| s.len() as u64);
        let keyexpr = Wireexpr {
            body: WireexprVariant::WireexprLocal(WireexprLocal {
    ...
    fn undecl_token(id: u64) -> UndeclToken {
        UndeclToken {
            id,
            ..UndeclToken::default()
        }
    ...
    fn declare_envelope_decl_token(d: DeclToken) -> Declare {
        Declare {
    ...
    fn declare_envelope_undecl_token(u: UndeclToken) -> Declare {
        Declare {
