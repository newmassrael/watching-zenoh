...
mod tests {
    use super::*;
    // PushOwnedVariant is named only by the push-builder coverage tests
    // (the type's producers moved to wz-session-core::push_build).
    #[cfg(feature = "codec-push")]
    use wz_codecs::push::PushOwnedVariant;
    // DeclareOwnedVariant is named by the declare-builder coverage tests
    // (matching `CodecZenohDecl*` / `CodecZenohUndecl*` arms); the producers
