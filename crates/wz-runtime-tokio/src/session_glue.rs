...
///
/// **Key sourcing**. `cookie_signing_key` is supplied by the
/// production caller through SessionInitParams; the key MUST be
/// kept per-process and at least 32 bytes per RFC §5.M (a 32-byte
/// key matches HMAC-SHA256's block-size optimum so the inner +
/// outer hash skip the key-expansion step). The integration test
/// uses a fixed 32-byte test vector so the cookie bytes are
/// reproducible.
///
/// **Materialised on the Accepting side only**. The Initiator
/// receives the cookie as part of the inbound InitAck wire bytes
/// and stages it for the OpenSyn echo via
/// `SessionInitParams::cookie`. R62 lands the generator helper;
/// the inbound-parse path that pulls the cookie off the wire and
/// stages it into the SessionInitParams is a later round (R63
/// inbound parser pass).
pub fn generate_cookie_hmac_sha256(
    cookie_signing_key: &[u8],
    peer_zid: &[u8],
) -> Vec<u8> {
    // `new_from_slice` accepts a key of any non-zero length. RFC
    // §5.M mandates >=32 bytes; we panic on shorter keys so the
    // misuse is loud at construction site (vs. wire-decoding-time
    // mystery rejects on the peer side).
    assert!(
        cookie_signing_key.len() >= 32,
        "cookie_signing_key must be >= 32 bytes per RFC §5.M (got {})",
        cookie_signing_key.len()
    );
    let mut mac = <Hmac<Sha256> as Mac>::new_from_slice(cookie_signing_key)
        .expect("HMAC-SHA256 accepts any non-zero key length");
    mac.update(peer_zid);
    // 32-byte HMAC output truncated to 16 bytes (cookie wire width).
...
pub struct SessionInitParams {
    ...
    /// outbound cookie. MUST be >= 32 bytes per RFC §5.M. Initiator
    /// side does not consume this field; the cookie value flows
    /// inbound from the peer's InitAck instead.
...
mod tests {
    ...
    /// mandates >= 32 bytes; we panic so the misuse cannot slip
    /// through into a peer reject at wire-decode time).
    #[test]
    #[should_panic(expected = "cookie_signing_key must be >= 32 bytes")]
    fn cookie_hmac_short_key_panics() {
        let _ = generate_cookie_hmac_sha256(&[0xAA; 16], &[0x01, 0x02]);
    }
