...
///
/// A value type rather than a borrow because it rides [`IterationEvent`],
/// which is `Copy` by contract (one observer callback fans the same event to
/// several consumers without reconstructing it), and because the peer it names
/// is GONE by the time an observer sees it — there is no slot left to borrow
/// from. That is the whole reason a departure has to carry its identity rather
/// than a table index: the index is already recycled.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// [`IterationEvent::MulticastPeerLost`].
///
