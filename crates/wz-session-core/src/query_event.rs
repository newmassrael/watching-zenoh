...
impl<'a> ReplyEmitter<'a> {
    ...
    /// `ReplyEmitter` instances.
    ///
    /// R311dx — `codec-request`-gated: the `&mut QueryResponder`
    /// parameter type now shares the registry's `codec-request` gate
    /// (the only caller, `fire_matching_queryables`, is itself
    /// `codec-request`-bound). The `query-queryable`-OFF `_marker` arm
    /// stays compile-time scaffolding only — `query-queryable` implies
    /// `codec-request`, so the constructor is reachable only when both
    /// are on; the OFF arm preserves Rust's "all public-API types must
    /// be nameable across builds" contract without ever materialising.
