...
//! ## Both edges (R311y802)
//!
//! The rise alone is satisfied by an implementation that latches, and on the
//! query plane a latched `true` is the more expensive error: a `Querier` whose
//! matching status never falls keeps issuing gets that nobody is left to answer.
//! The fall is driven by KILLING pico, so the retraction wz acts on is generated
//! by ZENOHD noticing the face close rather than by anything pico put on the
//! wire — the same distinction the subscriber-plane fixture's header states.
//!
...
//!
//! MEASURED for the fall too: deleting the `matching_watches` re-evaluation from
//! `QueryableRegistry`'s `UndeclQueryable` arm reds THIS file at 25s and leaves
//! the subscriber-plane witness green at 0.26s, and the subscriber-side deletion
//! does the mirror image. Each leg is bound to its own registry.
...
fn a_wz_querier_matching_listener_behind_zenohd_learns_of_a_pico_queryable() {
    ...
    // ── THE FALL (R311y802), the query-plane twin of the subscriber fixture's.
    //    Without it this file passes against an implementation that latches the
    //    status on the first matching queryable and never lowers it, and the
    //    query plane is where that would matter most: a `Querier` whose matching
    //    status is stuck at true keeps issuing gets nobody answers.
    //
    //    pico is KILLED rather than asked to undeclare, so the retraction wz acts
    //    on is generated by ZENOHD noticing the face close, not by anything pico
    //    put on the wire.
    let _ = pico_child.child_mut().kill();
    let _ = pico_child.child_mut().wait();

    let fall = format!("QUERIER MATCHING STATUS keyexpr='{QUERIER_KEY}' matching=false");
    ...
    if let Err(seen) = fell {
        panic!(
            "wz's QUERIER matching listener reported matching=true behind zenohd but \
             never matching=false within {:?}, after the only matching queryable's \
             process was killed. Either zenohd did not propagate the face-close \
             retraction, or wz LATCHED the querier status on first sight.\n\
             --- wz-ap-demo stderr ---\n{seen}{demo_captured}\n\
             --- z_queryable stdout ---\n{pico_captured}",
            MATCHING_TIMEOUT,
        );
