...
// R2423 (open-debt item 688) — this fixture used to carry a `_writer` field: the
// drive task below names `opened.inbound` / `.actions` / `.engine` / `.clock`,
// so Rust 2021's disjoint capture moved exactly those four fields into it and
// left `opened.writer_handle` to be dropped when `fixture()` returned. That drop
// SEALS the writer (`WriterHandle`'s `Drop`, R2367): the socket closed ~0.5 ms
// after Established, zenohd logged `RX task failed: … early eof`, and the two
// legs failed in the two ways a sealed queue produces — `publish` rejecting
// typed once the FSM had run (`500`), and, in the window BEFORE it did, a
// `declare_subscriber` that returned `Ok` for a `Declare` which vanished, so the
// SSE leg served `200 OK` and streamed nothing but keepalives.
//
// R2455 — the field is GONE because the hoist it implemented is no longer a
// thing a caller has to remember: `OpenedSession` implements `Drop`, so the task
// below captures the WHOLE session and the handle rides in with it. Holding the
// token by hand was the right repair for one caller; two more had the same shape
// and nobody had looked (open-debt item 695).
