...
//! precondition.
//!
//! ## Both edges, and why the second one needed a different driver
//!
//! R311y802 — this fixture asserted only the RISE for its first two rounds, and
//! an implementation that LATCHES (raises on the first matching declaration and
//! never lowers) satisfies every one of those assertions. The fall is the half
//! that separates the two, and it is driven by KILLING pico rather than by
//! asking it to undeclare. That is deliberate: the direct-peer sibling's
//! `z_sub -n 1` produces an explicit `UndeclSubscriber` from pico itself, so the
//! retraction wz acts on there is the PEER's. Here pico says nothing — zenohd
//! notices the face close and withdraws the subscription from the faces whose
//! interests it had matched, so the retraction is the ROUTER's. No other fixture
//! covers that arm.
//!
...
//!
//! MEASURED for the fall the same way: deleting the `matching_watches`
//! re-evaluation from `SubscriberRegistry`'s `UndeclSubscriber` arm leaves the
//! rise green (`matching=true` still logged) and times the fall out at 25s,
//! while the querier-plane twin stays green — so each leg is bound to its own
//! registry rather than to the pair.
...
fn a_wz_matching_listener_behind_zenohd_learns_of_a_pico_subscriber() {
    ...
    // ── THE FALL (R311y802). A witness that asserts only the rise passes just as
    //    well against an implementation that LATCHES: one that raises the status
    //    on the first matching declaration and never lowers it would satisfy
    //    every assertion above. This is the half that separates the two.
    //
    //    The driver is deliberately DIFFERENT from the direct-peer sibling's.
    //    `wz_matching_status_driven_by_pico_zsub` uses `z_sub -n 1`, so pico
    //    `z_drop`s its subscriber and an explicit UndeclSubscriber goes on the
    //    wire. Here pico is KILLED, so nothing pico sends causes the retraction:
    //    zenohd notices the face close and withdraws the subscription from the
    //    faces whose interests it had matched. The retraction wz must act on is
    //    therefore the ROUTER's, generated by a peer that said nothing — the arm
    //    no existing fixture covers.
    let _ = z_sub_child.child_mut().kill();
    let _ = z_sub_child.child_mut().wait();

    let fall = format!("MATCHING STATUS keyexpr='{PUBLISH_KEY}' matching=false");
    let fell = wait_for_substring(&mut demo_stderr_reader, &fall, MATCHING_TIMEOUT);
    ...
    if let Err(seen) = fell {
        panic!(
            "wz reported matching=true behind zenohd but never matching=false within \
