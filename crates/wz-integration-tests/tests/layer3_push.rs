...
//!
//! Scope (R102 first cut): default state — no flags, no extensions,
//! no keyexpr suffix, msg_put inner body with empty payload. Wire
//! shape is [0x1D, 0x00 (wireexpr.id VLE 0), 0x01 (msg_put header
//! MID 0x01), 0x00 (msg_put.payload_len VLE 0)] = 4 bytes.
//!
//! Why this matters: wz `Push::default()` defaults the inner-body
//! variant to `CodecZenohMsgPut(MsgPut::default())` per R88 variant-
//! default-uniformity; zenoh-pico's `_z_push_encode` reaches the
//! same wire bytes only when the fixture sets
...
//! definitions/network.c L22 with `._val = 5`). Together these
//! match the wz default state exactly.
...
fn zenoh_pico_encode_push_default() -> Vec<u8> {
    ...
    // where `_qos._val = 0` (NOT the upstream default of 5),
    // `_body._is_put = false` (would select the del branch), AND
    // `_key._mapping = 0 = _Z_KEYEXPR_MAPPING_LOCAL` which trips
    // `_z_wireexpr_is_local(&_key)` and sets the M flag in the
    // wire header. All three values must be patched before calling
    // the encoder so the wire shape matches wz Push::default()
    // (which defaults to M=0, put body, qos-default-elided).
    unsafe {
        ...
        // `_mapping = 0` is `_Z_KEYEXPR_MAPPING_LOCAL` per
        // protocol/core.h:151. `_z_wireexpr_is_local` returns true
        // for that value and the encoder sets M=1 on the header.
        // wz Push::default() leaves M=0 (push.scxml declares the
        // flag without a `value=` default), so the test fixture
        // assigns a non-local mapping value to keep the wire shapes
        // byte-equivalent. Filing as carry: investigate whether wz
        // codec should bake M=1 by default to match zenoh-pico's
        // semantic of "freshly-built wireexpr without prior
        // DECLARE is local to the sender".
        msg._key._mapping = 1;
...
fn layer3_push_default_byte_equivalent() {
    ...
    assert_eq!(
        wz,
        &[0x1D, 0x00, 0x01, 0x00],
        "default wire form: [push_header, wireexpr.id_vle, msg_put_header, payload_len_vle]"
    );
}
