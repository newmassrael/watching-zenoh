...
pub(crate) fn print_usage() {
    ...
    eprintln!("    --scout-autoconnect <roles>");
    eprintln!("                             the OTHER autoconnect plane: keep scouting the");
    eprintln!("                             multicast group and DIAL each responder whose role");
    eprintln!("                             is in <roles> (comma-separated router/peer/client,");
    eprintln!("                             or `none` for the empty set). zenoh");
    eprintln!("                             scouting/multicast/autoconnect; requires --peer and");
    eprintln!("                             the `scouting-active` feature. --autoconnect above is");
    eprintln!("                             the GOSSIP plane and the two are independent, which");
    eprintln!("                             is how zenoh keeps them.");
    eprintln!("    --scout-autoconnect-strategy <spec>");
    eprintln!("                             tie-break for the above (requires");
    eprintln!("                             --scout-autoconnect). Either one strategy for every");
    eprintln!("                             target (`always` / `greater-zid`) or a per-TARGET");
    eprintln!("                             table, `to_router=always,to_peer=greater-zid` --");
    eprintln!("                             zenoh scouting/multicast/autoconnect_strategy, whose");
    eprintln!("                             value is per discovered role and not per node.");
    ...
    eprintln!("                             unless --check-topology-external names them.");
    eprintln!("                             Nothing is resolved or probed. Requires the");
    eprintln!("                             `zenoh-config` build feature");
    eprintln!("    --check-topology-external <endpoint>");
    eprintln!("                             repeatable: an endpoint a node OUTSIDE this set");
    eprintln!("                             listens on, e.g. a stock zenohd somebody else");
    eprintln!("                             runs. Dials it answers stop being dangling, and");
    eprintln!("                             a set of nothing but clients stops being a defect");
    eprintln!("                             — that fragment is an ordinary deployment. Not");
    eprintln!("                             free: a declaration nothing dials, one the set");
    eprintln!("                             already listens on, and one that does not parse");
    eprintln!("                             are each reported, so widening cannot quietly");
    eprintln!("                             silence a typo. Every dial it answered is named");
    eprintln!("                             in the report, so a green verdict shows what it");
    eprintln!("                             assumed. Needs --check-topology");
