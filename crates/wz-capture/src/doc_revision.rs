...
pub const DOCUMENT_HISTORY: &[DocumentShape] = &[
    ...
    // R2630 (open-debt item 745) — THE SCOUTING LOSS, in the shared group.
    //
    // An ADDITION: `dropped_by_limits` gains `scouting`, the SCOUT and HELLO
    // datagrams a `frames_per_flow` ceiling evicted. `DissectionDrops::scouting`
    // has counted them since R311y651 and only the capture report rendered the
    // count, so a bounded census of a discovery flow reported a loss of nothing.
    // Nothing retires; families, planes and carries are revision 11's.
    DocumentShape {
        document: CENSUS,
        revision: 12,
        keys: CENSUS_R12_KEYS,
        retiring: &[],
        families: CENSUS_R11_FAMILIES,
        planes: CENSUS_R6_PLANES,
        carries: CENSUS_R11_CARRIES,
    ...
    // R2630 (open-debt item 745) — the census row's twin: this document embeds
    // the same `dropped_by_limits` rendering, so it gains `scouting` too.
    // Families and carries are revision 10's.
    DocumentShape {
        document: FIELDS,
        revision: 11,
        keys: FIELDS_R11_KEYS,
        retiring: &[],
        families: FIELDS_R10_FAMILIES,
        planes: &[],
        carries: FIELDS_R10_CARRIES,
    ...
    // R2630 (open-debt item 745) — revision 4 ADDS `scouting`, and retires
    // nothing.
    //
    // Two reasons under one key. The health object this door hands back embeds
    // `dropped_by_limits`, which now carries the scouting loss. And every
    // `datagram_flows[]` row of this document has emitted `scouting` since
    // R311y608 while no revision of this table declared it — see
    // [`SUMMARY_R4_KEYS`].
    DocumentShape {
        document: SUMMARY,
        revision: 4,
...
/// The census document's keys at revision 12 — revision 11's PLUS `scouting`.
///
/// R2630 (open-debt item 745). The shared loss group `dropped_by_limits` now
/// carries the scouting datagrams a `frames_per_flow` ceiling evicted — the count
/// `DissectionDrops::scouting` has held since R311y651 and only the capture
/// report's `drops` object rendered. Nothing retires.
pub const CENSUS_R12_KEYS: &[&str] = &[
    "a",
    "a_to_b",
    "aborted_capacity_overflow",
    "aborted_out_of_order",
...
/// The field document's keys at revision 11 — revision 10's (which are
/// revision 9's) PLUS `scouting`.
///
/// R2630 (open-debt item 745). The same addition as the census document's, for
/// the same reason: both embed the one `dropped_by_limits` rendering.
pub const FIELDS_R11_KEYS: &[&str] = &[
    "addr",
    "caps",
    "capture_reread",
    "carried",
...
/// The summary document's keys at revision 4 — revision 3's PLUS `scouting`.
///
/// R2630 (open-debt item 745). The health object this door hands back embeds
/// `dropped_by_limits`, which now carries the scouting loss.
///
/// ⚠ The key was ALREADY emitted and never declared: every `datagram_flows[]`
/// row of this document has carried `scouting` since R311y608, and no revision
/// of this table named it. This revision declares it for both reasons.
pub const SUMMARY_R4_KEYS: &[&str] = &[
    "bytes_absent",
    "caps",
    "capture_reported_drops",
    "completed",
