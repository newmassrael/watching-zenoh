...
# population; UNREACHED and NO_SYMBOL hold, as for every retirement above.
# R2626 — 34 -> 33: `time-hlc` went PARTIAL -> COMPLETE. Its last clause (the
# `drop_future_timestamp: true` arm) is built and cross-impl witnessed, which was
...
#
# R2820 — 12 -> 11. `adminspace-config-hotreload` RETIRES to COMPLETE: its
# last residual, the notification plane for library plugins, is built and
# witnessed by a pico over the wire. It was in `reached`, so this pin falls
# while UNREACHED and NO_SYMBOL hold at 2 and 2.
# READ off the census's own FAIL line, `reached: 11 against a pin of 12`.
#
# R2845 — 11 -> 12, the first RISE in this run of pins. `transport-stats`
# RE-ENTERS as PARTIAL: a multicast transport records no counter at all, where
...
# R2626 — 203 -> 197, FALLING because an atom RETIRED: `time-hlc` re-graded
# PARTIAL -> COMPLETE, so its whole reason leaves the graded population and takes
...
# R2642 — 177 -> 179, RISING, and rising for the ordinary reason a correction
# does: `config-mutate-runtime` stays PARTIAL, so its reason stays in this
# population, and R2642's CORRECTION cites two more tracked wz files by rooted
# path — the config registry it added and the gate that derives it. AMBIGUOUS
# holds at 38 (neither citation is ambiguous) and the unpinned upstream total
# moves 306 -> 307 on the one anchored upstream citation that correction makes.
# MEASURED by substituting origin's copy of that one reason back into the live
...
#
# R2800 — 83 -> 86, RISING: `storage-backend-filesystem` stays PARTIAL and
# gains three wz citations in an addendum recording that the read seam no
# longer generates its mirror: the seam's owned, fallible read
# (`storage_backend.rs` @ `fn get(&self, key: Option<&str>) ->
# Result<Vec<StoredData>, StorageReadError>;`), the direct exact-key reply
# (`storage_state.rs` @ `fn reply_set(&self, query_keyexpr: &str)`) and the
# external-writer witnesses (`storage_state.rs` @ `mod external_medium {`).
# DERIVED with this module's own `citation_audit` over that atom alone, before
# and after: (wz 5, ambiguous 0) -> (wz 8, ambiguous 0). ATTRIBUTED by the
...
#
# R2820 — 53 -> 44, FALLING by the whole of one atom:
# `adminspace-config-hotreload` is COMPLETE. The census was green at 53 on
# ab9497d1 and this atom's grade is the only one that moved since, so the
# fall is its wz citations and nothing else: 53 - 44 = 9, the census's own
# FAIL line, with PIN_AMBIGUOUS holding at 12 (the atom carried none).
#
# R2845 — 44 -> 46, RISING by the whole of one atom: `transport-stats`
# re-enters as PARTIAL (a multicast transport counts nothing). The census was
