...
layer_b2_regen_diff() {
    ...
    # R311y911 (open-debt item 424) — ASK WHETHER REGEN'S OUTPUT IS RECORDED,
    # not whether `out/` differs from HEAD.
    #
    # This was `git status --porcelain -- out/`, whose output includes
    # INDEX-vs-HEAD state: a staged file with a clean worktree prints `M `. So a
    # developer following `scripts/regen-codegen.sh`'s own instruction — "run
    # this and commit the resulting out/** changes" — who staged the regen and
    # then ran this gate before committing got eighteen files reported as stale
    # when regen had changed nothing at all. MEASURED: every marker was `M `,
    # index-only.
    #
    # Hosted CI never saw it, and never can: there the tree is a fresh checkout,
    # so HEAD, index and worktree agree and all three formulations answer the
    # same. That made it a gate that is wrong ONLY for the person it is meant to
    # help.
    #
    # The question that is right in both places: did regen leave anything that
    # is neither committed nor staged? Two halves, each named rather than a
    # column of `--porcelain` being parsed —
    #   * worktree vs index, which is a tracked file regen rewrote;
    #   * untracked, which is a file regen created and nothing has recorded.
    # A staged file whose content differs from regen's output still reds, via
    # the first half, because regen writes the WORKTREE: it is only the case
    # where the staged bytes ARE the regenerated bytes that now passes, and that
    # case is what the gate wants to be true.
    local regen_changed regen_new dirty
    regen_changed="$(git diff --name-only -- out/ 2>/dev/null)"
    regen_new="$(git ls-files --others --exclude-standard -- out/ 2>/dev/null)"
    dirty="$(printf '%s\n%s\n' "$regen_changed" "$regen_new" | sed '/^$/d')"
    if [[ -n "$dirty" ]]; then
...
# R2163 — the raising, the dims and the probe MOVED to
# `scripts/lib/uring-memlock.sh`, unchanged in behaviour, because this lane
# stopped being their only caller: `nondefault-tests-gate.sh`'s wide
# `wz-runtime-tokio` leg names `runtime-tokio-uring` too, and ran the same two
# registering tests with none of this provisioning. That file's header carries
# the measurement. What stays here is this lane's own VERDICT POLICY, which is
# the part that is genuinely C1br's.
# shellcheck source=scripts/lib/uring-memlock.sh
source "$(dirname "${BASH_SOURCE[0]}")/lib/uring-memlock.sh"
...
layer_c1br_uring_fixed_buffers() {
    ...
    # from here.
    uring_memlock_provision "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
    rc=$?
    if (( rc != 0 )); then
        why="$URING_MEMLOCK_WHY"
        if (( rc == 4 )); then
...
layer_c1bn_passive_dissection_features() {
    ...
    for name in \
        ...
        live_tap_tests::the_live_tap_preset_bounds_every_axis_it_has \
