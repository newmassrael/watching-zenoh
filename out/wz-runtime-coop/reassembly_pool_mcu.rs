...
/// `#[link_section]`.
pub const SECTION: &'static str = "sram2";
...
pub const FRAGMENT_BITMAP_WORDS: usize = (2 + 31) / 32;
...
pub const MAX_FRAGMENTS_PER_MESSAGE: u32 = 2;
...
pub const PER_PEER_QUOTA: u32 = 2;
...
impl Default for ReassemblyPoolMcu {
    fn default() -> Self {
        Self::new()
    }
}

impl ReassemblyPoolMcu {
...
impl Slot<CpuMut> {
    ...
    pub fn link_arm_tx(self, pool: &mut ReassemblyPoolMcu) {
    ...
    pub fn pool_return(self, pool: &mut ReassemblyPoolMcu) {
        pool.slot_states[self.idx] = SlotState::Free;
    }

    /// Read-only borrow of the slot's bytes.
    pub fn read<'a>(&'a self, pool: &'a ReassemblyPoolMcu) -> &'a [u8; SLOT_SIZE] {
        &pool.storage[self.idx].0
    }

    /// Mutable borrow of the slot's bytes.
    pub fn write<'a>(&'a mut self, pool: &'a mut ReassemblyPoolMcu) -> &'a mut [u8; SLOT_SIZE] {
...
impl Slot<DmaArmedRx> {
    ...
    /// edge.
    ///
    /// Takes `&mut ReassemblyPoolMcu` because a pointer valid for writes
    /// has to be derived from a mutable borrow.
    pub fn dma_armed_rx_ptr(&self, pool: &mut ReassemblyPoolMcu) -> *mut u8 {
...
impl Slot<DmaArmedRx> {
    ...
    /// Calling it early publishes memory the peripheral still owns.
    pub unsafe fn dma_start_rx(self, pool: &mut ReassemblyPoolMcu) {
...
impl Slot<CpuRef> {
    ...
    /// Calling it early publishes memory the peripheral still owns.
    pub unsafe fn mutate_in_place(self, pool: &mut ReassemblyPoolMcu) -> Slot<CpuMut> {
