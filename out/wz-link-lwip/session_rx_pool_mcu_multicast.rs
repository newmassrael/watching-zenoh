...
// generated-at: 0
// SCE-MAP: session_rx_pool_mcu_multicast.scxml:46 :: _forge_body
...
//
// Generated from document `session_rx_pool_mcu_multicast` with section `sram1`,
...
#[repr(C)]
pub struct SessionRxPoolMcuMulticast {
...
impl Default for SessionRxPoolMcuMulticast {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Slot<CpuMut> {
    ...
    pub fn link_arm_tx(self, pool: &mut SessionRxPoolMcuMulticast) {
    ...
    pub fn pool_return(self, pool: &mut SessionRxPoolMcuMulticast) {
        pool.slot_states[self.idx] = SlotState::Free;
    }

    /// Read-only borrow of the slot's bytes.
    pub fn read<'a>(&'a self, pool: &'a SessionRxPoolMcuMulticast) -> &'a [u8; SLOT_SIZE] {
        &pool.storage[self.idx].0
    }

    /// Mutable borrow of the slot's bytes.
    pub fn write<'a>(&'a mut self, pool: &'a mut SessionRxPoolMcuMulticast) -> &'a mut [u8; SLOT_SIZE] {
...
impl Slot<DmaArmedRx> {
    ...
    /// edge.
    ///
    /// Takes `&mut SessionRxPoolMcuMulticast` because a pointer valid for writes
    /// has to be derived from a mutable borrow.
    pub fn dma_armed_rx_ptr(&self, pool: &mut SessionRxPoolMcuMulticast) -> *mut u8 {
