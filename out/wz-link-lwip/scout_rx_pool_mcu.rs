...
// generated-at: 0
// SCE-MAP: scout_rx_pool_mcu.scxml:44 :: _forge_body
...
//
// Generated from document `scout_rx_pool_mcu` with section `sram1`,
...
impl Default for ScoutRxPoolMcu {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Slot<CpuMut> {
    ...
    pub fn link_arm_tx(self, pool: &mut ScoutRxPoolMcu) {
    ...
    pub fn pool_return(self, pool: &mut ScoutRxPoolMcu) {
        pool.slot_states[self.idx] = SlotState::Free;
    }

    /// Read-only borrow of the slot's bytes.
    pub fn read<'a>(&'a self, pool: &'a ScoutRxPoolMcu) -> &'a [u8; SLOT_SIZE] {
        &pool.storage[self.idx].0
    }

    /// Mutable borrow of the slot's bytes.
    pub fn write<'a>(&'a mut self, pool: &'a mut ScoutRxPoolMcu) -> &'a mut [u8; SLOT_SIZE] {
...
impl Slot<DmaArmedRx> {
    ...
    /// edge.
    ///
    /// Takes `&mut ScoutRxPoolMcu` because a pointer valid for writes
    /// has to be derived from a mutable borrow.
    pub fn dma_armed_rx_ptr(&self, pool: &mut ScoutRxPoolMcu) -> *mut u8 {
...
impl Slot<DmaArmedRx> {
    ...
    /// Calling it early publishes memory the peripheral still owns.
    pub unsafe fn dma_start_rx(self, pool: &mut ScoutRxPoolMcu) {
...
impl Slot<CpuRef> {
    ...
    /// Calling it early publishes memory the peripheral still owns.
    pub unsafe fn mutate_in_place(self, pool: &mut ScoutRxPoolMcu) -> Slot<CpuMut> {
