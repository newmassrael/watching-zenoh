...
// From: sources/session/multicast_peer.scxml
//
...
// SCE-MAP: multicast_peer.scxml:70 :: _machine
...
impl MulticastPeerEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [MulticastPeerEvent] = &[
        MulticastPeerEvent::PeerActivated,
        MulticastPeerEvent::PeerDiscovered,
        MulticastPeerEvent::PeerLost,
        MulticastPeerEvent::PeerRecycle,
    ];
}
...
pub struct MulticastPeerPolicy<A: MulticastPeerActions + 'static> {
    ...
    last_transition_source_state: MulticastPeerState,
}

impl<A: MulticastPeerActions + 'static> MulticastPeerPolicy<A> {
    pub fn new(actions: A) -> Self {
        Self {
            actions,
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: MulticastPeerState::Free,
        }
    }
...
impl<A: MulticastPeerActions + 'static> StatePolicy for MulticastPeerPolicy<A> {
    type State = MulticastPeerState;
    type Event = MulticastPeerEvent;
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            MulticastPeerState::Active => 2,
            MulticastPeerState::Discovered => 1,
            MulticastPeerState::Expired => 3,
            MulticastPeerState::Free => 0,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            MulticastPeerEvent::PeerActivated => "peer.activated",
            MulticastPeerEvent::PeerDiscovered => "peer.discovered",
            MulticastPeerEvent::PeerLost => "peer.lost",
            MulticastPeerEvent::PeerRecycle => "peer.recycle",
            MulticastPeerEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "peer.activated" => Some(MulticastPeerEvent::PeerActivated),
            "peer.discovered" => Some(MulticastPeerEvent::PeerDiscovered),
            "peer.lost" => Some(MulticastPeerEvent::PeerLost),
            "peer.recycle" => Some(MulticastPeerEvent::PeerRecycle),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            MulticastPeerState::Active => "Active",
    ...
    #[doc = "SCE-MAP: multicast_peer.scxml:70 :: _machine"]
// SCE-MAP: multicast_peer.scxml:70 :: _machine
    fn execute_entry_actions(&mut self, state: Self::State, engine: &mut sce_rust_runtime::Engine<Self>, path_child: Option<Self::State>) {
        ...
        let _ = path_child;
        match state {
            MulticastPeerState::Discovered => {
                // SCE-MAP: multicast_peer.scxml:89 :: Discovered :: _state_body
                // W3C SCXML 3.8: onentry block 1/1
                // Labeled block allows actions to break out on error (W3C 3.8: error stops block)
                'action_block: {

            // W3C SCXML G.7: <sce:action name="init_rx_seq">
            self.actions.init_rx_seq();
                }
            }
            MulticastPeerState::Expired => {
                // SCE-MAP: multicast_peer.scxml:108 :: Expired :: _state_body
                // W3C SCXML 3.8: onentry block 1/1
                // Labeled block allows actions to break out on error (W3C 3.8: error stops block)
                'action_block: {

            // W3C SCXML G.7: <sce:action name="emit_peer_lost">
            self.actions.emit_peer_lost();
                }
            }
            _ => {}
        }
...
impl<A: MulticastPeerActions + 'static> MulticastPeerPolicy<A> {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: MulticastPeerState,
        event: MulticastPeerEvent,
        current_state: &mut MulticastPeerState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            MulticastPeerState::Active => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == MulticastPeerEvent::PeerLost {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = MulticastPeerState::Expired;
            ...
            MulticastPeerState::Discovered => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == MulticastPeerEvent::PeerActivated {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = MulticastPeerState::Active;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
            ...
            MulticastPeerState::Expired => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == MulticastPeerEvent::PeerRecycle {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = MulticastPeerState::Free;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            MulticastPeerState::Free => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == MulticastPeerEvent::PeerDiscovered {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = MulticastPeerState::Discovered;
                            *transition_taken = true;
                        return true;
                }
                false
