// SCE-MAP: decl_final:28 :: _forge_body
...
impl<'a> Default for DeclFinal<'a> {
    fn default() -> Self {
        Self {
            header: 0x1au8,
...
// `DeclFinal<'a>` above is a zero-copy view borrowing the decode
...
// `DeclFinalOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `DeclFinalOwned<Heap>` holds growable
// containers with the declared capacities advisory, `DeclFinalOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `DeclFinalOwned` alone is the build's
...
// `let v: DeclFinalOwned = DeclFinalOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct DeclFinalOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub extensions: Option<S::List<ExtEntryOwned<S>, 16>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclFinalOwned<S> {
...
impl<'a> DeclFinal<'a> {
    ...
    /// [`DeclFinalOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<DeclFinalOwned<S>, CodecError> {
        Ok(DeclFinalOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<DeclFinalOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclFinalOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<DeclFinal<'_>, CodecError> {
        Ok(DeclFinal {
            header: self.header,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<DeclFinalOwned<D>, CodecError> {
