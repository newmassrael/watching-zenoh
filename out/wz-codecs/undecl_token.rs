...
impl<'a> Default for UndeclToken<'a> {
    fn default() -> Self {
        Self {
            header: 0x07u8,
            id: Default::default(),
...
// `UndeclToken<'a>` above is a zero-copy view borrowing the decode
...
// `UndeclTokenOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `UndeclTokenOwned<Heap>` holds growable
// containers with the declared capacities advisory, `UndeclTokenOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `UndeclTokenOwned` alone is the build's
...
// `let v: UndeclTokenOwned = UndeclTokenOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct UndeclTokenOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> UndeclTokenOwned<S> {
...
impl<'a> UndeclToken<'a> {
    ...
    /// [`UndeclTokenOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<UndeclTokenOwned<S>, CodecError> {
        Ok(UndeclTokenOwned {
            header: self.header,
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<UndeclTokenOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> UndeclTokenOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<UndeclToken<'_>, CodecError> {
        Ok(UndeclToken {
            header: self.header,
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<UndeclTokenOwned<D>, CodecError> {
