// SCE-MAP: msg_del:76 :: _forge_body
...
// `MsgDel<'a>` above is a zero-copy view borrowing the decode
...
// `MsgDelOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `MsgDelOwned<Heap>` holds growable
// containers with the declared capacities advisory, `MsgDelOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `MsgDelOwned` alone is the build's
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct MsgDelOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> MsgDelOwned<S> {
...
impl<'a> MsgDel<'a> {
    ...
    /// [`MsgDelOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<MsgDelOwned<S>, CodecError> {
        Ok(MsgDelOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<MsgDelOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> MsgDelOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<MsgDel<'_>, CodecError> {
        Ok(MsgDel {
            header: self.header,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<MsgDelOwned<D>, CodecError> {
