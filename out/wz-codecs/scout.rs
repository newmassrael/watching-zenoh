...
// `Scout<'a>` above is a zero-copy view borrowing the decode
...
// `ScoutOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `ScoutOwned<Heap>` holds growable
// containers with the declared capacities advisory, `ScoutOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `ScoutOwned` alone is the build's
...
pub struct ScoutOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub version: u8,
    pub cbyte: u8,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> ScoutOwned<S> {
...
impl<'a> Scout<'a> {
    ...
    /// [`ScoutOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<ScoutOwned<S>, CodecError> {
        Ok(ScoutOwned {
    ...
    pub fn try_into_owned(self) -> Result<ScoutOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> ScoutOwned<S> {
    ...
    pub fn as_borrowed(&self) -> Scout<'_> {
        Scout {
            version: self.version,
            cbyte: self.cbyte,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<ScoutOwned<D>, CodecError> {
