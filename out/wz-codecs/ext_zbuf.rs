// SCE-MAP: ext_zbuf:25 :: _forge_body
...
// `ExtZbuf<'a>` above is a zero-copy view borrowing the decode
...
// `ExtZbufOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `ExtZbufOwned<Heap>` holds growable
// containers with the declared capacities advisory, `ExtZbufOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `ExtZbufOwned` alone is the build's
...
// `let v: ExtZbufOwned = ExtZbufOwned { .. };`
...
pub struct ExtZbufOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub value_len: u64,
    pub value: S::Bytes<32>,
}

#[allow(dead_code)]
impl<'a> ExtZbuf<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`ExtZbufOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<ExtZbufOwned<S>, CodecError> {
        Ok(ExtZbufOwned {
            value_len: self.value_len,
            value: <S::Bytes<32> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.value)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<ExtZbufOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> ExtZbufOwned<S> {
    ...
    pub fn as_borrowed(&self) -> ExtZbuf<'_> {
        ExtZbuf {
            value_len: self.value_len,
            value: ::sce_forge_runtime::codec::SceByteBuf::as_slice(&self.value),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<ExtZbufOwned<D>, CodecError> {
