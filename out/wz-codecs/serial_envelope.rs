// SCE-MAP: serial_envelope:57 :: _forge_body
...
impl<'a> SerialEnvelope<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let crc32 = {
            ...
            let _v = raw[0] as u32 | ((raw[1] as u32) << 8) | ((raw[2] as u32) << 16) | ((raw[3] as u32) << 24);
            cursor.advance(4)?;
            _v
        };
        Ok(Self {
    ...
    pub const MAX_ENCODED_BYTES: usize = 1508;
...
// `SerialEnvelope<'a>` above is a zero-copy view borrowing the decode
...
// `SerialEnvelopeOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `SerialEnvelopeOwned<Heap>` holds growable
// containers with the declared capacities advisory, `SerialEnvelopeOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `SerialEnvelopeOwned` alone is the build's
...
// `let v: SerialEnvelopeOwned = SerialEnvelopeOwned { .. };`
...
pub struct SerialEnvelopeOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub payload_len: u16,
    pub payload: S::Bytes<1500>,
    pub crc32: u32,
}
...
impl<'a> SerialEnvelope<'a> {
    ...
    /// [`SerialEnvelopeOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<SerialEnvelopeOwned<S>, CodecError> {
        Ok(SerialEnvelopeOwned {
            header: self.header,
            payload_len: self.payload_len,
            payload: <S::Bytes<1500> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.payload)?,
            crc32: self.crc32,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<SerialEnvelopeOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> SerialEnvelopeOwned<S> {
    ...
    pub fn as_borrowed(&self) -> SerialEnvelope<'_> {
        SerialEnvelope {
            header: self.header,
            payload_len: self.payload_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<SerialEnvelopeOwned<D>, CodecError> {
