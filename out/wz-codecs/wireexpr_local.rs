// SCE-MAP: wireexpr_local:108 :: _forge_body
...
// `WireexprLocal<'a>` above is a zero-copy view borrowing the decode
...
// `WireexprLocalOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `WireexprLocalOwned<Heap>` holds growable
// containers with the declared capacities advisory, `WireexprLocalOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `WireexprLocalOwned` alone is the build's
...
// `let v: WireexprLocalOwned = WireexprLocalOwned { .. };`
...
pub struct WireexprLocalOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u64,
    pub suffix_len: Option<u64>,
...
impl<'a> WireexprLocal<'a> {
    ...
    /// [`WireexprLocalOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<WireexprLocalOwned<S>, CodecError> {
        Ok(WireexprLocalOwned {
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<WireexprLocalOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> WireexprLocalOwned<S> {
    ...
    pub fn as_borrowed(&self) -> WireexprLocal<'_> {
        WireexprLocal {
            id: self.id,
            suffix_len: self.suffix_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<WireexprLocalOwned<D>, CodecError> {
