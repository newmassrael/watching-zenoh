...
impl<'a> Push<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 1203;
...
impl<'a> PushVariant<'a> {
    ...
    pub fn try_into_owned(self) -> Result<PushOwnedVariant, CodecError> {
        Ok(match self {
            PushVariant::CodecZenohMsgPut(_b) => PushOwnedVariant::CodecZenohMsgPut(_b.try_into_owned()?),
            PushVariant::CodecZenohMsgDel(_b) => PushOwnedVariant::CodecZenohMsgDel(_b.try_into_owned()?),
            PushVariant::Default { tag, body } => PushOwnedVariant::Default { tag, body: body.try_into_owned()? },
        })
    }
}

#[cfg(feature = "alloc")]
impl PushOwnedVariant {
...
impl<'a> Push<'a> {
    ...
    /// [`PushOwned`]. Call at a decode boundary when the
