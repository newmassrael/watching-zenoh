// SCE-MAP: interest:53 :: _forge_body
...
use super::interest_body::InterestBody;
...
impl<'a> Interest<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let interest_id = cursor.read_vle_u64()?;
        let body = if (header & 0x20u8) != 0 || (header & 0x40u8) != 0 {
            Some(InterestBody::decode(cursor)?)
        } else {
            None
        };
    ...
    pub fn encode<S: SceSink>(&self, w: &mut S) -> Result<(), CodecError> {
        ...
        w.write_vle_u64(self.interest_id)?;
...
// `Interest<'a>` above is a zero-copy view borrowing the decode
...
// `InterestOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `InterestOwned<Heap>` holds growable
// containers with the declared capacities advisory, `InterestOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `InterestOwned` alone is the build's
...
// `let v: InterestOwned = InterestOwned { .. };`
...
use super::interest_body::InterestBodyOwned;
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct InterestOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub interest_id: u64,
    pub body: Option<InterestBodyOwned<S>>,
    pub extensions: Option<S::List<ExtEntryOwned<S>, 16>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> InterestOwned<S> {
...
impl<'a> Interest<'a> {
    ...
    /// [`InterestOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<InterestOwned<S>, CodecError> {
        Ok(InterestOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<InterestOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> InterestOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<Interest<'_>, CodecError> {
        Ok(Interest {
            header: self.header,
            interest_id: self.interest_id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<InterestOwned<D>, CodecError> {
