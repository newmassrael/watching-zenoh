...
impl<'a> Default for UndeclQueryable<'a> {
    fn default() -> Self {
        Self {
            header: 0x05u8,
            id: Default::default(),
...
// `UndeclQueryable<'a>` above is a zero-copy view borrowing the decode
...
// `UndeclQueryableOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `UndeclQueryableOwned<Heap>` holds growable
// containers with the declared capacities advisory, `UndeclQueryableOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `UndeclQueryableOwned` alone is the build's
...
// `let v: UndeclQueryableOwned = UndeclQueryableOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct UndeclQueryableOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> UndeclQueryableOwned<S> {
...
impl<'a> UndeclQueryable<'a> {
    ...
    /// [`UndeclQueryableOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<UndeclQueryableOwned<S>, CodecError> {
        Ok(UndeclQueryableOwned {
            header: self.header,
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<UndeclQueryableOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> UndeclQueryableOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<UndeclQueryable<'_>, CodecError> {
        Ok(UndeclQueryable {
            header: self.header,
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<UndeclQueryableOwned<D>, CodecError> {
