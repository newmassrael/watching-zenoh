// SCE-MAP: timestamp:42 :: _forge_body
...
impl<'a> Timestamp<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let zid_len = cursor.read_vle_u64()?;
        let zid = {
            let _n = zid_len as usize;
...
// `Timestamp<'a>` above is a zero-copy view borrowing the decode
...
// `TimestampOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `TimestampOwned<Heap>` holds growable
// containers with the declared capacities advisory, `TimestampOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `TimestampOwned` alone is the build's
...
// `let v: TimestampOwned = TimestampOwned { .. };`
...
pub struct TimestampOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub time: u64,
    pub zid_len: u64,
    pub zid: S::Bytes<16>,
}
...
impl<'a> Timestamp<'a> {
    ...
    /// [`TimestampOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<TimestampOwned<S>, CodecError> {
        Ok(TimestampOwned {
            time: self.time,
    ...
    pub fn try_into_owned(self) -> Result<TimestampOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> TimestampOwned<S> {
    ...
    pub fn as_borrowed(&self) -> Timestamp<'_> {
        Timestamp {
            time: self.time,
            zid_len: self.zid_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<TimestampOwned<D>, CodecError> {
