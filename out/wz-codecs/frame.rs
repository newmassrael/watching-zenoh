...
// `Frame<'a>` above is a zero-copy view borrowing the decode
...
// `FrameOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `FrameOwned<Heap>` holds growable
// containers with the declared capacities advisory, `FrameOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `FrameOwned` alone is the build's
...
pub struct FrameOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub sn: u64,
...
impl<'a> Frame<'a> {
    ...
    /// [`FrameOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<FrameOwned<S>, CodecError> {
        Ok(FrameOwned {
            sn: self.sn,
    ...
    pub fn try_into_owned(self) -> Result<FrameOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> FrameOwned<S> {
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<FrameOwned<D>, CodecError> {
