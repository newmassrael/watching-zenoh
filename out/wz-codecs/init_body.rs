// SCE-MAP: init_body:60 :: _forge_body
...
impl<'a> InitBody<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>, s: u8, a: u8) -> Result<Self, CodecError> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 159;

    /// Encode `self` into the caller-owned sink. Returns
    /// `CodecError::BufferOverflow` from a bounded sink when the
    /// destination has insufficient remaining capacity; growable
    /// sinks (e.g. `VecSink`) are effectively infallible.
    pub fn encode<S: SceSink>(&self, w: &mut S, s: u8, a: u8) -> Result<(), CodecError> {
    ...
    /// primary `encode`.
    #[cfg(feature = "alloc")]
    pub fn encode_to_vec(&self, s: u8, a: u8) -> Vec<u8> {
        let mut _sce_v: Vec<u8> = Vec::with_capacity(Self::MAX_ENCODED_BYTES);
        let mut _sce_sink = VecSink::new(&mut _sce_v);
        self.encode(&mut _sce_sink, s, a)
...
// `InitBody<'a>` above is a zero-copy view borrowing the decode
...
// `InitBodyOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `InitBodyOwned<Heap>` holds growable
// containers with the declared capacities advisory, `InitBodyOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `InitBodyOwned` alone is the build's
...
// `let v: InitBodyOwned = InitBodyOwned { .. };`
...
pub struct InitBodyOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub version: u8,
    pub cbyte: u8,
    pub zid: S::Bytes<16>,
    pub sn_res: Option<u8>,
    pub batch_size: Option<u16>,
    pub cookie_len: Option<u64>,
    pub cookie: Option<S::Bytes<128>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> InitBodyOwned<S> {
...
impl<'a> InitBody<'a> {
    ...
    /// [`InitBodyOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<InitBodyOwned<S>, CodecError> {
        Ok(InitBodyOwned {
            ...
            cookie: self.cookie.map(<S::Bytes<128> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<InitBodyOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> InitBodyOwned<S> {
    ...
    pub fn as_borrowed(&self) -> InitBody<'_> {
        InitBody {
            version: self.version,
            cbyte: self.cbyte,
            ...
            batch_size: self.batch_size,
            cookie_len: self.cookie_len,
            cookie: self.cookie.as_ref().map(::sce_forge_runtime::codec::SceByteBuf::as_slice),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<InitBodyOwned<D>, CodecError> {
