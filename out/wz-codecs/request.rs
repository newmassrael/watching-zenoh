...
#[derive(Debug, Clone, PartialEq)]
pub enum RequestOwnedVariant {
    CodecZenohMsgPut(MsgPutOwned),
    CodecZenohMsgDel(MsgDelOwned),
    CodecZenohQuery(QueryOwned),
    Default {
        tag: u8,
        body: QueryOwned,
    },
...
impl<'a> RequestVariant<'a> {
    ...
    pub fn try_into_owned(self) -> Result<RequestOwnedVariant, CodecError> {
        Ok(match self {
            RequestVariant::CodecZenohMsgPut(_b) => RequestOwnedVariant::CodecZenohMsgPut(_b.try_into_owned()?),
            RequestVariant::CodecZenohMsgDel(_b) => RequestOwnedVariant::CodecZenohMsgDel(_b.try_into_owned()?),
            RequestVariant::CodecZenohQuery(_b) => RequestOwnedVariant::CodecZenohQuery(_b.try_into_owned()?),
            RequestVariant::Default { tag, body } => RequestOwnedVariant::Default { tag, body: body.try_into_owned()? },
        })
    }
}

#[cfg(feature = "alloc")]
impl RequestOwnedVariant {
...
impl<'a> Request<'a> {
    ...
    /// [`RequestOwned`]. Call at a decode boundary when the
