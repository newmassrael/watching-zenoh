...
// `Fragment<'a>` above is a zero-copy view borrowing the decode
...
// `FragmentOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `FragmentOwned<Heap>` holds growable
// containers with the declared capacities advisory, `FragmentOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `FragmentOwned` alone is the build's
...
// `let v: FragmentOwned = FragmentOwned { .. };`
...
pub struct FragmentOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub sn: u64,
...
impl<'a> Fragment<'a> {
    ...
    /// [`FragmentOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<FragmentOwned<S>, CodecError> {
        Ok(FragmentOwned {
            sn: self.sn,
    ...
    pub fn try_into_owned(self) -> Result<FragmentOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> FragmentOwned<S> {
    ...
    pub fn as_borrowed(&self) -> Fragment<'_> {
        Fragment {
            sn: self.sn,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<FragmentOwned<D>, CodecError> {
