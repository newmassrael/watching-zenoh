...
impl<'a> Join<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 49;
...
// `Join<'a>` above is a zero-copy view borrowing the decode
...
// `JoinOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `JoinOwned<Heap>` holds growable
// containers with the declared capacities advisory, `JoinOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `JoinOwned` alone is the build's
...
pub struct JoinOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub version: u8,
    pub cbyte: u8,
    pub zid: S::Bytes<16>,
    pub sn_res: Option<u8>,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> JoinOwned<S> {
...
impl<'a> Join<'a> {
    ...
    /// [`JoinOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<JoinOwned<S>, CodecError> {
        Ok(JoinOwned {
    ...
    pub fn try_into_owned(self) -> Result<JoinOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> JoinOwned<S> {
    ...
    pub fn as_borrowed(&self) -> Join<'_> {
        Join {
            version: self.version,
            cbyte: self.cbyte,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<JoinOwned<D>, CodecError> {
