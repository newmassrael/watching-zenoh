// SCE-MAP: decl_subscriber:39 :: _forge_body
...
impl<'a> Default for DeclSubscriber<'a> {
    fn default() -> Self {
        Self {
            header: 0x02u8,
            id: Default::default(),
...
// `DeclSubscriber<'a>` above is a zero-copy view borrowing the decode
...
// `DeclSubscriberOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `DeclSubscriberOwned<Heap>` holds growable
// containers with the declared capacities advisory, `DeclSubscriberOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `DeclSubscriberOwned` alone is the build's
...
// `let v: DeclSubscriberOwned = DeclSubscriberOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct DeclSubscriberOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclSubscriberOwned<S> {
...
impl<'a> DeclSubscriber<'a> {
    ...
    /// [`DeclSubscriberOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<DeclSubscriberOwned<S>, CodecError> {
        Ok(DeclSubscriberOwned {
            header: self.header,
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<DeclSubscriberOwned, CodecError> {
