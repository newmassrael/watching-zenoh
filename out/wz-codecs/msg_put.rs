...
// `MsgPut<'a>` above is a zero-copy view borrowing the decode
...
// `MsgPutOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `MsgPutOwned<Heap>` holds growable
// containers with the declared capacities advisory, `MsgPutOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `MsgPutOwned` alone is the build's
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct MsgPutOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> MsgPutOwned<S> {
...
impl<'a> MsgPut<'a> {
    ...
    /// [`MsgPutOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<MsgPutOwned<S>, CodecError> {
        Ok(MsgPutOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<MsgPutOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> MsgPutOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<MsgPut<'_>, CodecError> {
        Ok(MsgPut {
            header: self.header,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<MsgPutOwned<D>, CodecError> {
