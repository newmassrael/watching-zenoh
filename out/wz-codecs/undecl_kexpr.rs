...
impl<'a> Default for UndeclKexpr<'a> {
    fn default() -> Self {
        Self {
            header: 0x01u8,
            id: Default::default(),
...
// `UndeclKexpr<'a>` above is a zero-copy view borrowing the decode
...
// `UndeclKexprOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `UndeclKexprOwned<Heap>` holds growable
// containers with the declared capacities advisory, `UndeclKexprOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `UndeclKexprOwned` alone is the build's
...
// `let v: UndeclKexprOwned = UndeclKexprOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct UndeclKexprOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> UndeclKexprOwned<S> {
...
impl<'a> UndeclKexpr<'a> {
    ...
    /// [`UndeclKexprOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<UndeclKexprOwned<S>, CodecError> {
        Ok(UndeclKexprOwned {
            header: self.header,
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<UndeclKexprOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> UndeclKexprOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<UndeclKexpr<'_>, CodecError> {
        Ok(UndeclKexpr {
            header: self.header,
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<UndeclKexprOwned<D>, CodecError> {
