// SCE-MAP: decl_kexpr:28 :: _forge_body
...
impl<'a> DeclKexpr<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let keyexpr = Wireexpr::decode(cursor, (header >> 5) & 0x1, 0x1u8)?;
...
// `DeclKexpr<'a>` above is a zero-copy view borrowing the decode
...
// `DeclKexprOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `DeclKexprOwned<Heap>` holds growable
// containers with the declared capacities advisory, `DeclKexprOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `DeclKexprOwned` alone is the build's
...
// `let v: DeclKexprOwned = DeclKexprOwned { .. };`
...
pub struct DeclKexprOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclKexprOwned<S> {
...
impl<'a> DeclKexpr<'a> {
    ...
    /// [`DeclKexprOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<DeclKexprOwned<S>, CodecError> {
        Ok(DeclKexprOwned {
            header: self.header,
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<DeclKexprOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclKexprOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<DeclKexpr<'_>, CodecError> {
        Ok(DeclKexpr {
            header: self.header,
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<DeclKexprOwned<D>, CodecError> {
