// SCE-MAP: open_body:49 :: _forge_body
...
impl<'a> OpenBody<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>, a: u8) -> Result<Self, CodecError> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 155;

    /// Encode `self` into the caller-owned sink. Returns
    /// `CodecError::BufferOverflow` from a bounded sink when the
    /// destination has insufficient remaining capacity; growable
    /// sinks (e.g. `VecSink`) are effectively infallible.
    pub fn encode<S: SceSink>(&self, w: &mut S, a: u8) -> Result<(), CodecError> {
    ...
    /// primary `encode`.
    #[cfg(feature = "alloc")]
    pub fn encode_to_vec(&self, a: u8) -> Vec<u8> {
        let mut _sce_v: Vec<u8> = Vec::with_capacity(Self::MAX_ENCODED_BYTES);
        let mut _sce_sink = VecSink::new(&mut _sce_v);
        self.encode(&mut _sce_sink, a)
...
// `OpenBody<'a>` above is a zero-copy view borrowing the decode
...
// `OpenBodyOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `OpenBodyOwned<Heap>` holds growable
// containers with the declared capacities advisory, `OpenBodyOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `OpenBodyOwned` alone is the build's
...
// `let v: OpenBodyOwned = OpenBodyOwned { .. };`
...
pub struct OpenBodyOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub lease: u64,
    pub initial_sn: u64,
    pub cookie_len: Option<u64>,
...
impl<'a> OpenBody<'a> {
    ...
    /// [`OpenBodyOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<OpenBodyOwned<S>, CodecError> {
        Ok(OpenBodyOwned {
            lease: self.lease,
    ...
    pub fn try_into_owned(self) -> Result<OpenBodyOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> OpenBodyOwned<S> {
    ...
    pub fn as_borrowed(&self) -> OpenBody<'_> {
        OpenBody {
            lease: self.lease,
            initial_sn: self.initial_sn,
            cookie_len: self.cookie_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<OpenBodyOwned<D>, CodecError> {
