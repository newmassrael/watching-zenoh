// SCE-MAP: wireexpr_nonlocal:95 :: _forge_body
...
// `WireexprNonlocal<'a>` above is a zero-copy view borrowing the decode
...
// `WireexprNonlocalOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `WireexprNonlocalOwned<Heap>` holds growable
// containers with the declared capacities advisory, `WireexprNonlocalOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `WireexprNonlocalOwned` alone is the build's
...
// `let v: WireexprNonlocalOwned = WireexprNonlocalOwned { .. };`
...
pub struct WireexprNonlocalOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u64,
    pub suffix_len: Option<u64>,
...
impl<'a> WireexprNonlocal<'a> {
    ...
    /// [`WireexprNonlocalOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<WireexprNonlocalOwned<S>, CodecError> {
        Ok(WireexprNonlocalOwned {
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<WireexprNonlocalOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> WireexprNonlocalOwned<S> {
    ...
    pub fn as_borrowed(&self) -> WireexprNonlocal<'_> {
        WireexprNonlocal {
            id: self.id,
            suffix_len: self.suffix_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<WireexprNonlocalOwned<D>, CodecError> {
