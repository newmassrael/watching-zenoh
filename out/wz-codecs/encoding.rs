// SCE-MAP: encoding:56 :: _forge_body
...
// `Encoding<'a>` above is a zero-copy view borrowing the decode
...
// `EncodingOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `EncodingOwned<Heap>` holds growable
// containers with the declared capacities advisory, `EncodingOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `EncodingOwned` alone is the build's
...
// `let v: EncodingOwned = EncodingOwned { .. };`
...
pub struct EncodingOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub packed_id: u32,
    pub schema_len: Option<u64>,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> EncodingOwned<S> {
...
impl<'a> Encoding<'a> {
    ...
    /// [`EncodingOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<EncodingOwned<S>, CodecError> {
        Ok(EncodingOwned {
    ...
    pub fn try_into_owned(self) -> Result<EncodingOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> EncodingOwned<S> {
    ...
    pub fn as_borrowed(&self) -> Encoding<'_> {
        Encoding {
            packed_id: self.packed_id,
            schema_len: self.schema_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<EncodingOwned<D>, CodecError> {
