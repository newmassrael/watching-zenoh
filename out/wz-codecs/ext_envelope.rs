// SCE-MAP: ext_envelope:56 :: _forge_body
...
pub struct ExtEnvelope<'a> {
    pub header_flags: u8,
    pub extensions: HeaplessVec<ExtEntry<'a>, 8>,
}

#[allow(dead_code)]
impl<'a> ExtEnvelope<'a> {
...
// `ExtEnvelope<'a>` above is a zero-copy view borrowing the decode
...
// `ExtEnvelopeOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `ExtEnvelopeOwned<Heap>` holds growable
// containers with the declared capacities advisory, `ExtEnvelopeOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `ExtEnvelopeOwned` alone is the build's
...
// `let v: ExtEnvelopeOwned = ExtEnvelopeOwned { .. };`
...
pub struct ExtEnvelopeOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header_flags: u8,
    pub extensions: S::List<ExtEntryOwned<S>, 16>,
}

#[allow(dead_code)]
impl<'a> ExtEnvelope<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`ExtEnvelopeOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<ExtEnvelopeOwned<S>, CodecError> {
        Ok(ExtEnvelopeOwned {
    ...
    pub fn try_into_owned(self) -> Result<ExtEnvelopeOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> ExtEnvelopeOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<ExtEnvelope<'_>, CodecError> {
        Ok(ExtEnvelope {
            header_flags: self.header_flags,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<ExtEnvelopeOwned<D>, CodecError> {
