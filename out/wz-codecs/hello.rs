...
impl<'a> Hello<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 8795;
...
// `Hello<'a>` above is a zero-copy view borrowing the decode
...
// `HelloOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `HelloOwned<Heap>` holds growable
// containers with the declared capacities advisory, `HelloOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `HelloOwned` alone is the build's
...
pub struct HelloOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub version: u8,
    pub cbyte: u8,
    pub zid: S::Bytes<16>,
    pub num_locators: Option<u64>,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> HelloOwned<S> {
...
impl<'a> Hello<'a> {
    ...
    /// [`HelloOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<HelloOwned<S>, CodecError> {
        Ok(HelloOwned {
    ...
    pub fn try_into_owned(self) -> Result<HelloOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> HelloOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<Hello<'_>, CodecError> {
        Ok(Hello {
            version: self.version,
            cbyte: self.cbyte,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<HelloOwned<D>, CodecError> {
