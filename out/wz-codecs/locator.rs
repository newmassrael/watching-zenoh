// SCE-MAP: locator:32 :: _forge_body
...
impl<'a> Locator<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let locator_len = cursor.read_vle_u64()?;
        let locator = {
    ...
    pub const MAX_ENCODED_BYTES: usize = 137;
    ...
    pub fn encode<S: SceSink>(&self, w: &mut S) -> Result<(), CodecError> {
        ...
        w.write_bytes(self.locator.as_bytes())?;
        Ok(())
    }
...
// `Locator<'a>` above is a zero-copy view borrowing the decode
...
// `LocatorOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `LocatorOwned<Heap>` holds growable
// containers with the declared capacities advisory, `LocatorOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `LocatorOwned` alone is the build's
...
// `let v: LocatorOwned = LocatorOwned { .. };`
...
pub struct LocatorOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub locator_len: u64,
    pub locator: S::Str<128>,
}

#[allow(dead_code)]
impl<'a> Locator<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`LocatorOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<LocatorOwned<S>, CodecError> {
        Ok(LocatorOwned {
            locator_len: self.locator_len,
            locator: <S::Str<128> as ::sce_forge_runtime::codec::SceStr>::from_view(self.locator)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<LocatorOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> LocatorOwned<S> {
    ...
    pub fn as_borrowed(&self) -> Locator<'_> {
        Locator {
            locator_len: self.locator_len,
            locator: ::sce_forge_runtime::codec::SceStr::as_str(&self.locator),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<LocatorOwned<D>, CodecError> {
