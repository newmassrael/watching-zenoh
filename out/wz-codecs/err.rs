...
impl<'a> Err<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 858;
...
// `Err<'a>` above is a zero-copy view borrowing the decode
...
// `ErrOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `ErrOwned<Heap>` holds growable
// containers with the declared capacities advisory, `ErrOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `ErrOwned` alone is the build's
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct ErrOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> ErrOwned<S> {
...
impl<'a> Err<'a> {
    ...
    /// [`ErrOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<ErrOwned<S>, CodecError> {
        Ok(ErrOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<ErrOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> ErrOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<Err<'_>, CodecError> {
        Ok(Err {
            header: self.header,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<ErrOwned<D>, CodecError> {
