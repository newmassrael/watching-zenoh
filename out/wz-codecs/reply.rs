...
impl<'a> ReplyVariant<'a> {
    ...
    pub fn try_into_owned(self) -> Result<ReplyOwnedVariant, CodecError> {
        Ok(match self {
            ReplyVariant::CodecZenohMsgPut(_b) => ReplyOwnedVariant::CodecZenohMsgPut(_b.try_into_owned()?),
            ReplyVariant::CodecZenohMsgDel(_b) => ReplyOwnedVariant::CodecZenohMsgDel(_b.try_into_owned()?),
            ReplyVariant::Default { tag, body } => ReplyOwnedVariant::Default { tag, body: body.try_into_owned()? },
        })
    }
}

#[cfg(feature = "alloc")]
impl ReplyOwnedVariant {
...
impl<'a> Reply<'a> {
    ...
    /// [`ReplyOwned`]. Call at a decode boundary when the
