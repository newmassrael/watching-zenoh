...
#[cfg(feature = "alloc")]
#[derive(Debug, Clone, PartialEq)]
pub struct ExtEntryOwned {
    pub header: u8,
    pub body: ExtEntryOwnedVariant,
}
...
impl<'a> ExtEntryVariant<'a> {
    ...
    pub fn try_into_owned(self) -> Result<ExtEntryOwnedVariant, CodecError> {
        Ok(match self {
            ...
            ExtEntryVariant::CodecZenohExtZbuf(_b) => ExtEntryOwnedVariant::CodecZenohExtZbuf(_b.try_into_owned()?),
...
impl<'a> ExtEntry<'a> {
    ...
    /// [`ExtEntryOwned`]. Call at a decode boundary when the
