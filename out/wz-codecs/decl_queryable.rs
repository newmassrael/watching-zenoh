// SCE-MAP: decl_queryable:33 :: _forge_body
...
impl<'a> Default for DeclQueryable<'a> {
    fn default() -> Self {
        Self {
            header: 0x04u8,
            id: Default::default(),
...
// `DeclQueryable<'a>` above is a zero-copy view borrowing the decode
...
// `DeclQueryableOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `DeclQueryableOwned<Heap>` holds growable
// containers with the declared capacities advisory, `DeclQueryableOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `DeclQueryableOwned` alone is the build's
...
// `let v: DeclQueryableOwned = DeclQueryableOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct DeclQueryableOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclQueryableOwned<S> {
...
impl<'a> DeclQueryable<'a> {
    ...
    /// [`DeclQueryableOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<DeclQueryableOwned<S>, CodecError> {
        Ok(DeclQueryableOwned {
            header: self.header,
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<DeclQueryableOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> DeclQueryableOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<DeclQueryable<'_>, CodecError> {
        Ok(DeclQueryable {
            header: self.header,
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<DeclQueryableOwned<D>, CodecError> {
