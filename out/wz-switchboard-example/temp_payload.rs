...
// source-hash: 9ed884e88fb5707b0f3d69d7c4ba940de1fd31fdfd28818789c268a25c0dfd46
...
pub struct TempPayload<'a> {
    pub celsius_centi: u16,
    pub sensor_id_len: u64,
    pub sensor_id: &'a str,
    pub raw_len: u64,
    pub raw: &'a [u8],
}
...
impl<'a> TempPayload<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let sensor_id_len = cursor.read_vle_u64()?;
        let sensor_id = {
            let _n = sensor_id_len as usize;
        ...
        let raw_len = cursor.read_vle_u64()?;
        let raw = {
            let _n = raw_len as usize;
            let raw = cursor.peek_slice(_n)?;
            let _v = raw;
            cursor.advance(_n)?;
            _v
    ...
    pub fn encode<S: SceSink>(&self, w: &mut S) -> Result<(), CodecError> {
        ...
        w.write_u8((self.celsius_centi >> 8 & 0xFF) as u8)?;
        w.write_u8((self.celsius_centi & 0xFF) as u8)?;
        w.write_vle_u64(self.sensor_id_len)?;
        w.write_bytes(self.sensor_id.as_bytes())?;
        w.write_vle_u64(self.raw_len)?;
...
// `TempPayload<'a>` above is a zero-copy view borrowing the decode
...
// `TempPayloadOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `TempPayloadOwned<Heap>` holds growable
// containers with the declared capacities advisory, `TempPayloadOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `TempPayloadOwned` alone is the build's
...
// `let v: TempPayloadOwned = TempPayloadOwned { .. };`
...
pub struct TempPayloadOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub celsius_centi: u16,
    pub sensor_id_len: u64,
    pub sensor_id: S::Str<64>,
    pub raw_len: u64,
    pub raw: S::Bytes<64>,
}
...
impl<'a> TempPayload<'a> {
    ...
    /// [`TempPayloadOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<TempPayloadOwned<S>, CodecError> {
        Ok(TempPayloadOwned {
            celsius_centi: self.celsius_centi,
            sensor_id_len: self.sensor_id_len,
            sensor_id: <S::Str<64> as ::sce_forge_runtime::codec::SceStr>::from_view(self.sensor_id)?,
            raw_len: self.raw_len,
            raw: <S::Bytes<64> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.raw)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<TempPayloadOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> TempPayloadOwned<S> {
    ...
    pub fn as_borrowed(&self) -> TempPayload<'_> {
        TempPayload {
            celsius_centi: self.celsius_centi,
            sensor_id_len: self.sensor_id_len,
            sensor_id: ::sce_forge_runtime::codec::SceStr::as_str(&self.sensor_id),
            raw_len: self.raw_len,
            raw: ::sce_forge_runtime::codec::SceByteBuf::as_slice(&self.raw),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<TempPayloadOwned<D>, CodecError> {
